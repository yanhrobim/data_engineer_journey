...
def valor_total_gasto_p_cliente_in_json(dataframe_com_valor_total: pd.DataFrame, path_para_json: str):
    ...
    pd.DataFrame(agrupando_p_cliente).rename(columns={None: "valor_total_gasto"}).to_json(path_or_buf=f"{path_para_json}/resumo_clientes.json",
                                                                                      orient='records', indent=4, force_ascii=False)
    return print(f"O JSON foi criado com sucesso! Localizado em: '{path_para_json}/resumo_clientes.json'.")
