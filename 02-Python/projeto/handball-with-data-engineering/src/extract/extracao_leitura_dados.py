...
def leitura_budeshandball_csv(nome_pasta_com_arquivo_csv: str, nome_arquivo: str) -> pd.DataFrame:
    ...
    try:
        ...
        if len(dados) == 0:
            print("O arquivo CSV passado parece ter colunas, porém não tem valores (dados)! Verifique o parâmetro que você passou para a função de leitura!")

    except pandas.errors.EmptyDataError:
        print("O arquivo CSV passado não possui dados! Verifique o parâmetro que você passou para a função de leitura!")
    
    except ValueError:
        print("\nO caminho passado ao método: pd.read_csv() não encontra um arquivo CSV! Revise o caminho passado!")
