...
def lidar_com_colunas_null(budeshandball_csv: pd.DataFrame):

    colunas_com_null = budeshandball_csv.columns[budeshandball_csv.isnull().any()].tolist() 
    ...
    for coluna in colunas_com_null:
        ...
        if budeshandball_csv[coluna].dtype == float:
            coluna_numerica_sem_null =  budeshandball_csv[coluna].fillna(0)
            budeshandball_csv[coluna] = coluna_numerica_sem_null

        print(budeshandball_csv[coluna].dtypes)

        if budeshandball_csv[coluna].dtype == 'str': 
            coluna_str_sem_null = budeshandball_csv[coluna].fillna("valor_faltante")
            budeshandball_csv[coluna] = coluna_str_sem_null
        
    return budeshandball_csv
...
# if str(types_colum[colunas_com_null]) == 'float64':
#     leitura[colunas_com_null].fillna(value=0)
#     print(leitura['Nome'])